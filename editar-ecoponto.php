...
else
{
    ...
    $id = isset($_GET['id']) && !empty(trim($_GET['id'])) ? $_GET['id'] : '';
    ...
    $sql = $pdo->prepare("SELECT * FROM cadastro_ecoponto WHERE id_usuario = :i");
    $sql->bindValue(":i", $_SESSION['id_usuario']);
    $sql->execute();
    ...
    foreach($sql->fetchAll() as $row)
    {
        ...
        $sql = $pdo->prepare("SELECT * FROM tipos_residuos WHERE id_ecoponto = :i");
        $sql->bindValue(":i", $row['id_cadastro_ecoponto']);
        $sql->execute();

        foreach($sql->fetchAll() as $ln)
        {
            $dadosEcoponto['tipos_residuos'][] = $ln['desc_residuo'];
        }
    }
}
?>
...
<body>
  ...
  <div class="container">
    ...
    <div class="d-grid gap-2 mx-auto">
      <h3 class="h3-cadastro">Editar Ecoponto</h3>

        <form action="./cadastro-ecoponto.php" method="POST">
            <p class="topicos">ID ECOPONTO</p>
            <input class="form-control" name="id_ecoponto" type="text" placeholder="ID ECOPONTO" readonly>
