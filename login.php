...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    if($sql->rowCount() > 0)
    {
      ...
      if(password_verify($senha, $dados['senha']))
      {
        ...
        header('Location: ./logado.php?logado=true');
      }
      else
      {
        header('Location: ./login.php?noPass=true');
      }
    }
    else
    {
      header('Location: ./cadastro.php?noExiste=true');
    }
}
?>
...
<head>
  ...
  <link rel="stylesheet" href="./css/login_index/style.css">
  ...
  <div class="container">
    ...
    <div class="d-grid gap-2 mx-auto">
      <form action="./login.php" method="POST">
        <input class="form-btn__login form-control" type="email" name="email" placeholder="E-mail" required>
        <input class="form-btn__login form-control" type="password" name="senha" placeholder="Senha" required>
        <input id="btn-login" class="form-control login form-btn btn" type="submit" value="enviar">
      </form>
      <p class="p-container forget-password">Esqueceu a senha ? Clique aqui.</p>
      <?php
    ...
    elseif(isset($_GET['noPass']) && !empty($_GET['noPass']))
    {
    ?>
      <div class="container">
          <div class="alert alert-danger" role="alert">
              <span class="card-title">Erro!</span>
              <p>Falha ao Logar! E-mail ou Senha incorretos!</p>
          </div>
      </div>
    <?php
    }
    elseif(isset($_GET['noLogin']) && !empty($_GET['noLogin']))
    {
    ?>
      <div class="container">
    ...
    ?>
      <footer>
        <p class="p-container p-footer forget-password">Não tenho uma conta. Toque aqui para criar uma agora</p>
      </footer>
    </div>
  </div>
